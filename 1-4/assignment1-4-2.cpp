...
int main(int argc, char *argv[]) {
  ...
  cv::Mat dst_img, msk_img, bkg_img;
  ...
  bkg_img = cv::imread("background.jpg", 1);
  cv::resize(bkg_img, bkg_img, s);
  ...
  while (loop_flag) {
    ...
    /* hatena */
    frame.copyTo(bkg_img, msk_img);

    cv::imshow("Input", frame);
    cv::imshow("FG", bkg_img); // ここをバックグランドイメージに変更
